...
Deliver the first end-to-end public browsing path from real app data to the `/` route. This task should prove the public visibility rules and render an intentional empty state without depending on the canonical meme detail route.
...
- This slice starts as an independent browse-first surface and can ship useful progress before the canonical detail route exists.
- Once the detail route exists, opening a meme from the feed should use that route as the destination.
